package servlet.criarPerfil;

import dao.UtilizadorDAO;
import dao.VeterinarioDAO;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import model.Utilizador.Utilizador;

import java.io.IOException;

@WebServlet("/criarVeterinario")
public class CriarVeterinarioServlet extends HttpServlet {

    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

        try {
            Utilizador u = new Utilizador(true, false, false, false, req.getParameter("username"), req.getParameter("password"));

            UtilizadorDAO udao = new UtilizadorDAO();
            int id = udao.inserir(u);

            VeterinarioDAO vdao = new VeterinarioDAO();
            vdao.inserirMinimo(id);

            resp.sendRedirect("gerente/criarAtualizarVet.jsp?ok=1");

        } catch (Exception e) {
            throw new ServletException(e);
        }
    }
}
